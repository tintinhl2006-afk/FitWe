...
export default function LiveWorkoutPage({ params }: { params: Promise<{ sessionId: string }> }) {
  ...
  const getExerciseIcon = (equipment: string | null | undefined, muscleGroup: string) => {
    ...
    if (lowerGroup === "cardio") return <Activity size={16} className="text-blue-600 dark:text-blue-400" />;
    ...
    if (lowerEq === "barra" || lowerEq === "máquina" || lowerEq === "polea") return <Target size={16} className="text-indigo-600 dark:text-indigo-400" />;
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-24 transition-colors">
      <div className="sticky top-0 z-10 bg-slate-900 dark:bg-slate-950 border-b dark:border-slate-800 text-white shadow-md px-4 py-4">
        <div className="flex items-center justify-between max-w-md mx-auto">
          <div>
            <h1 className="font-bold text-lg leading-tight line-clamp-1">{session.routine?.name || "Entrenamiento Libre"}</h1>
            <div className="flex items-center gap-1.5 text-indigo-300 dark:text-indigo-400 text-sm font-medium mt-0.5">
          ...
          <button
            ...
            className="flex items-center gap-1 bg-red-500 hover:bg-red-600 text-white px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors shadow-sm disabled:opacity-70"
          >
...
function SetRow({ set, index, historicalSet, onUpdate, weightUnit }: any) {
  ...
  const placeholderWeight = historicalSet?.weight !== undefined ? historicalSet.weight.toString() : (weightUnit === 'lbs' ? 'Lbs' : 'Kg');
  const placeholderReps = historicalSet?.reps !== undefined ? historicalSet.reps.toString() : (isCardio ? "Minutos" : "Reps");

  return (
    <div className={cn(
      "flex items-center px-2 py-2 rounded-xl transition-colors",
      set.isCompleted 
      ...
      {!isCardio && !isBodyweight && (
        <div className="flex-1 px-1">
          <input
            ...
            className={cn(
              "w-full text-center py-2.5 rounded-lg border focus:ring-2 focus:ring-indigo-500 focus:outline-none transition-colors text-base font-semibold",
              set.isCompleted 
